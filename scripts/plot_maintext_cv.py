...
def draw_panel(ax, ds, state_idx, ylabel=None, meas_vals=None, meas_err=None,
    ...
    ax.text(0.035, 0.83, tag, transform=ax.transAxes, fontsize=10.5,
            fontweight="bold", va="top", ha="left")


def cv_legend(fig, measurement_label, y=-0.015):
    """Per-column legend: each dataset is one column — header states Tuning/Validation,
    then Model / EnKF / band / measurement. matplotlib fills legends column-major, so
    handles are grouped by dataset to keep each batch in its own column."""
    handles = []
    for ds in COLS:                       # column-major: one full column per dataset
        handles += [
            Patch(facecolor=COLZ[ds], edgecolor="black", lw=0.6, label=f"{ds}  ({ROLE[ds]})"),
            Line2D([0], [0], color=COLZ[ds], lw=MODEL_LW, ls="-", label="Model"),
            Line2D([0], [0], color=COLZ[ds], lw=ENKF_LW, ls="--", label="EnKF"),
            Patch(facecolor=COLZ[ds], alpha=0.18, label=r"$\pm2\sigma$ (10 seeds)"),
            Line2D([0], [0], color=COLZ[ds], marker=MKR[ds], lw=0, markersize=7,
                   markeredgecolor="black", markeredgewidth=0.6, label=measurement_label),
        ]
    fig.legend(handles=handles, loc="lower center", ncol=4, fontsize=10.5,
               frameon=False, bbox_to_anchor=(0.5, y), columnspacing=3.0,
...
# ── Figure 2: Asparagine — 2 x 2 over P1..P4 ────────────────────────────────────
...
for i, ds in enumerate(COLS):
    ...
    draw_panel(ax, ds, si, ylabel="Asparagine Concentration (mM)",
               ...
               letter=LETTERS[i], show_role=True, show_ylabel=(i % 2 == 0))
    if i >= 2:
        ax.set_xlabel("Time (hours)", fontsize=11, fontweight="bold")
cv_legend(fig, "Measurement", y=-0.008)
